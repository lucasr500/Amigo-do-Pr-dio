...
export default function HomeCondominioHub({ refreshKey }: Props) {
  const [hydrated, setHydrated] = useState(false);
  const [nome, setNome] = useState<string | null>(null);
  const [result, setResult] = useState<HealthScoreResult | null>(null);

  useEffect(() => {
    setNome(getProfile()?.nomeCondominio ?? null);
        ...
        <div className="px-4 pb-3 pt-3.5">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 truncate text-[14px] font-semibold leading-snug text-navy-800">
                {nome ?? "Condomínio em acompanhamento"}
              </p>
            </div>
            <span
          ...
          <div className="mt-2.5 flex items-center gap-3">
            <p className="font-display text-[24px] font-semibold leading-none tracking-tight text-navy-800">
              {result.percentage}%
            </p>
            <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-navy-100/60">
              <div
