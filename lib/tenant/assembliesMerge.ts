// Merge puro de assembleias e itens de pauta (local ↔ remoto).
// Sem React, sem localStorage, sem rede. Determinístico e testável.
// Espelha a regra de lib/tenant/agendaMerge.ts:
...
export function mergeAssemblies(local: Assembly[], remote: Assembly[]): Assembly[] {
  return mergeById(local, remote);
}

export function mergeAgendaItems(
  local: AssemblyAgendaItem[],
  remote: AssemblyAgendaItem[]
): AssemblyAgendaItem[] {
  return mergeById(local, remote);
}
