...
- [x] **Fases 87/88/89A — Onboarding, Plano Supabase e Stubs de Arquitetura:** `lib/session.ts`: `KEYS.ONBOARDING_COMPLETE`, `isFirstRun()`, `markFirstRunComplete()`. `OnboardingFlow.tsx` (novo): overlay 4 etapas, bottom sheet mobile / card desktop, palette Navy/Cream, sem dependências externas — salva perfil + memória + flag via localStorage. `app/page.tsx`: `showOnboarding` state, detecção `isFirstRun()` no primeiro useEffect, overlay via `dynamic()` antes de BottomNav, seção "Conta e sincronização" com badge "Dados salvos" + card "Sincronização em nuvem — Em breve". `docs/supabase-backend-plan.md` (novo): plano técnico completo — schema, RLS, auth, sync snapshot-based, LGPD, bundle, checklist produção, rollback. `supabase/migrations/001_initial_schema.sql` (novo): tabelas `profiles` + `app_snapshots` com triggers, índice único, RLS. Stubs: `lib/supabase/client.ts`, `lib/auth/authClient.ts`, `lib/sync/syncEngine.ts` — tipos e TODOs sem SDK real. `@supabase/supabase-js` não instalado. package.json e lockfiles inalterados. TypeScript zero erros. Bundle / 224 kB (margem 6 kB). Sem login funcional, sem sync real, sem nova rota.
- [x] **Fase 86 — Agenda Mensal v2 — Ícones Operacionais:** `Response.tsx`: removido estado `isTyping` (sempre false após Fase 84) e as três condições `!isTyping &&` do JSX — código morto eliminado sem mudança visual. Separador visual (`border-t border-navy-100/60 pt-4`) adicionado entre resposta principal e blocos auxiliares — torna explícito que Próximo passo, Contexto do prédio, Base legal e Dica prática são camadas separadas da resposta, não sua continuação. Wrapper dos blocos: `mt-4 space-y-2.5` → `mt-5 border-t border-navy-100/60 pt-4 space-y-3`. Section: `pb-6` → `pb-8` no mobile (unifica com sm). Action pills: `mt-2.5` → `mt-3`. Todos os blocos de conteúdo, disclaimers, contexto local, fallback, like, copiar, salvar próximo passo e histórico preservados. Sem alteração de KB, motor de busca, Agenda, Home, Saúde Operacional ou qualquer lib. Bundle / ≤ 230 kB. TypeScript zero erros.
- [x] **Fase 84 — Confiabilidade do Assistente e Saúde Operacional acionável:** `Response.tsx`: efeito typewriter character-by-character (14ms/char) substituído por exibição imediata do texto completo + `animate-fade-in` — elimina percepção de truncamento no mobile, especialmente em respostas observadas durante a animação; cursor piscante removido do JSX; todos os blocos auxiliares (Próximo passo, Contexto do prédio, Base legal, Dica prática, Checklist, Comunicado, disclaimers) preservados. `HomeCondominioHub.tsx`: microcopy expandida abaixo da frase diagnóstica — "Considera dados cadastrados, alertas ativos, próximos passos e revisão semanal." + "Para melhorar: resolva alertas, conclua passos e mantenha os prazos atualizados." — cálculo, barra, badge e estética inalterados. `AgendaMensal.tsx` não foi alterado. Sem IA, backend, login, billing, push, recorrência, nova aba, nova rota, KB, guidance, motor de busca ou simuladores alterados. Bundle / mantido ≤ 230 kB. TypeScript zero erros.
- [x] **Fase 83 — Agenda Central e Home sem redundância:** `AgendaMensal.tsx` (novo): calendário mensal compacto na Home com grade de 7 colunas, destaque para hoje e dia selecionado, dots em dias com eventos, detalhe do dia ao tocar, botão "+ Agendar neste dia" navega para Ferramentas → Agenda. Fontes de dados combinadas: eventos manuais de `getAgendaEvents()` + vencimentos calculados do `getMemoriaOperacional()` (AVCB, Seguro, Mandato, AGO, Dedetização, Caixa d'água, Elevador, Extintores, SPDA, Elétrica). Na Home: tipo e título truncado sem nota para eventos manuais; labels genéricos para vencimentos automáticos. Posicionado entre HomeCondominioHub e GuidancePanel quando há dados. `HomeAcaoHub` enxugado: removido bloco "Próxima data" (coberto por AgendaMensal), removido CTA "Ver agenda →", renomeado título "O que fazer agora" → "Próximos passos do prédio", adicionado CTA "Perguntar ao Assistente →" com prop `onNavigateToAssistente`. `BottomNav`: label "Condomínio" → "Minha Conta" (id `condominio` e lógica interna inalterados). `QuickAccessCards`: reorganizado para tema-primeiro: grade de cards com título de tema + "Ver perguntas →"; ao clicar no tema, exibe lista de 5 perguntas sugeridas por tema (definidas inline via `THEME_QUESTIONS`, sem tocar lib/data.ts, KB ou motor); ao clicar na pergunta, preenche o input normalmente. Hierarquia da Home com dados: HomeCondominioHub → AgendaMensal → GuidancePanel → HomeAcaoHub (enxugado). Sem IA, backend, login real, push, recorrência, integração externa, nova rota, nova aba. Bundle / 224 kB (+2 kB). /admin 204 kB (inalterado). TypeScript zero erros.
- [x] **Fase 82 — Documento técnico de IA futura no Assistente (fase documental):** fase exclusivamente documental — nenhum arquivo de código, componente, lib ou bundle alterado. Criado `docs/plano-futuro-ia-assistente.md`: documento central e mais atualizado sobre possibilidade futura de IA no Assistente, com 15 seções (resumo executivo, quando usar/não usar IA, arquitetura proposta com fluxo determinístico-primeiro, backend necessário, guardrails, prompt base apenas como documentação, dados permitidos/proibidos, logs e privacidade, estimativa de custo por volume, critérios antes de implementar, estratégia de rollout em 4 etapas, riscos mapeados com severidade, decisão atual). Decisão registrada: IA não será implementada antes de telemetria ativa com dados reais, taxa de fallback > 30% comprovada, backend seguro, política de privacidade, termos de uso e revisão jurídica. GuidancePanel, saúde operacional, agenda, simuladores e alertas de prazos permanecem determinísticos. IA permanece hipótese futura. Runtime inalterado. Sem dependência nova.
- [x] **Fase 81 — Assistente contextual sem IA (extensão de contexto local):** `Response.tsx` estendido com dois novos casos em `getLocalContextNotice()`: **barulho/ocorrência** (detecção por palavras-chave; texto sugere registrar como ocorrência e criar próximo passo — sem dados locais, sem texto livre); **agenda futura** (importa `getUpcomingAgendaEvents(90)` e filtra por tipo relacionado — para manutenção: manutencao/dedetizacao/caixa_agua/extintores/vistoria; para assembleia: assembleia/reuniao — sem expor título, nota ou data exata). Tipo `LocalContextNotice.contextType` estendido para incluir `"barulho"` e `"agenda"`. Telemetria `local_context_notice_shown` absorve novos valores sem alteração em `lib/telemetry.ts`. Fallback atualizado: texto mais honesto + três sugestões em texto discreto (reformular; registrar ocorrência; criar próximo passo) — sem botão, sem navegação, sem alterar `app/page.tsx`. Motor de busca, KB, GuidancePanel, HomeAcaoHub e AgendaPredio inalterados. Sem IA, RAG, backend, dados sensíveis enviados. Bundle inalterado: / 222 kB, /admin 204 kB. TypeScript zero erros.
- [x] **Fase 80 — Conta & Dados (reorganização interna da aba Condomínio):** aba reorganizada em seções conceituais sem feature nova. Título interno muda de "Dados do prédio" para "Conta & Dados" quando há dados cadastrados; BottomNav permanece "Condomínio". Quatro rótulos de seção discretos adicionados em `app/page.tsx` (apenas quando `hasCondominioData`): "Dados do prédio", "Vencimentos e rotinas", "Histórico operacional", "Backup e segurança". Dois blocos estáticos informativos adicionados sempre: "Suporte e termos" (avisa que suporte/termos/política virão antes da versão comercial) e "Preferências futuras" (descreve conta, cor, notificações e assinatura como hipóteses). Nenhum componente existente alterado. `id="revisao-mensal"` preservado. Sem login, billing, backend, termos completos, suporte ativo ou conta real. "Minha Conta" permanece hipótese futura. TypeScript zero erros. Bundle inalterado.
- [x] **Fase 79 — Agenda do Prédio v1:** lista leve de eventos operacionais futuros integrada ao grupo "Rotina do síndico". `AgendaPredio.tsx` (novo): formulário (título, data, tipo, nota, opção de criar próximo passo vinculado), 13 tipos de evento, lista ordenada por urgência com cores, concluir/excluir com confirmação, histórico de concluídos colapsável. `lib/session.ts`: `AgendaEventType`, `AgendaEvent`, `KEYS.AGENDA`, 7 funções, `"agenda"` no union `Pendencia.origem`. Backup v4 com `agenda?: AgendaEvent[]`; v1/v2/v3 importam sem quebra (KEYS.AGENDA limpo ao importar versão antiga). `HomeAcaoHub`: evento mais urgente da agenda compete com datas monitoradas; seção "Próxima data" → "Próximo na agenda" quando agenda vence; CTA "Ver agenda →" adicionado. `TimelineOperacional`: concluídos entram como "Item da agenda concluído" (📅) sem título/nota. Telemetria: 3 eventos (`agenda_event_created/completed/deleted`) com type/days_until/has_note/has_linked_step — sem PII. TypeScript zero erros. Sem calendário mensal, push, nova aba, nova rota, modal obrigatório, recorrência, IA, login, backend.
- [x] **Fase 78 — Ferramentas como central de ações por categorias:** aba Ferramentas reorganizada de layout flat para menu de categorias clicáveis. Ao abrir a aba, o usuário vê 5 cards (Rotina do síndico / Comunicados / Simuladores / Checklists / Explorar por tema) — cada um com ícone, título, descrição e chevron. Ao clicar em um card, apenas o conteúdo daquela categoria é exibido, com botão discreto "← Voltar para ferramentas". Link da Home ("+ Registrar ocorrência") preservado: abre Ferramentas diretamente no grupo "Rotina do síndico" via `ANCHOR_TO_GROUP`. Duplicidade visual de "Comunicados" eliminada (cabeçalho externo de seção removido). `activeToolGroup` gerenciado por `useState` local em `app/page.tsx` — sem localStorage, sem query params, sem modal, sem nova rota. Reset para `null` ao sair da aba Ferramentas. Hipóteses futuras registradas em docs: Agenda do Prédio (lista de eventos do condomínio, integração com Home e Timeline, sem grade mensal de início), IA no Assistente (fallback ancorado na KB, não substituto do GuidancePanel, depende de backend e revisão jurídica), Condomínio/Conta (aba poderá evoluir para "Conta" quando houver login e billing). Bundle / 221 kB (inalterado). TypeScript zero erros. Sem IA, login, billing, backend, WhatsApp, nova aba, KB, guidance, backup schema, simuladores, checklists ou comunicados alterados.
- [x] **Fase 77 — Reset seguro, feedback de backup e validação do Assistente:** `clearAllData()` centralizada em `lib/session.ts` (remove todas as 18 chaves `amigo_*`, SSR-safe, sem alterar schema de backup); `BackupPanel` com "Novo condomínio / limpar dados" — fluxo idle → confirming (digitar "APAGAR") → done ("Dados apagados. Você pode começar um novo condomínio."), sem apagar sem confirmação explícita; feedback de exportação inline com nome do arquivo por 4 s; `Response.tsx` inspecionado — sem truncamento real por CSS, nenhuma alteração necessária; smoke test seções 16 e 17 com roteiro de reset e 5 perguntas do Assistente. Bundle 221 kB. TypeScript zero erros. Sem IA, login, billing, backend, WhatsApp, nova aba, KB, guidance, backup schema, simuladores ou motor de busca. Produto pré-beta interno: sem tráfego pago, sem teste com síndicos, sem launch externo.
- [x] **Fase 76B — Consolidar Home em dois hubs operacionais:** `HomeCondominioHub` (identidade + índice de saúde) e `HomeAcaoHub` (revisão semanal + próximos passos + próxima data + registrar ocorrência) substituem 6 componentes separados; aba Condomínio simplificada (SaudeOperacionalPanel full removido); bundle 221 kB (−3 kB vs Fase 76). TypeScript zero erros. Sem IA, login, billing, backend.
- [x] **Fase 76 — Saúde operacional compacta na Home e índice completo na aba Condomínio:** `SaudeOperacionalPanel` com `variant="compact"` na aba Início; variant full na aba Condomínio sem alteração. Bundle 224 kB. TypeScript zero erros. Sem IA.
- [x] **Fase 75 — Índice de saúde operacional determinístico:** `lib/health-score.ts` puro, `SaudeOperacionalPanel` na aba Condomínio com percentual 0–100%, 5 faixas qualitativas, barra, frase diagnóstica, até 5 fatores, até 3 sugestões, disclaimer operacional. Sem score jurídico, sem porcentagem "regular", sem compliance. Bundle 224 kB.
- [x] **Fase 74 — Ferramentas e checklists mais claros:** subtítulo de Ferramentas → "registrar, comunicar, simular e acompanhar"; ChecklistPanel disclaimer → orientativo, ajuste conforme convenção e realidade do condomínio.
...
## Fase 86 — Agenda Mensal v2: Ícones Operacionais (2026-05-23)

**Entregue:** `AgendaMensal` aprimorada com ícones visuais discretos por tipo de evento e vencimento. Único arquivo de produto alterado: `components/AgendaMensal.tsx`.

- `TYPE_ICONS`: mapeamento de 13 tipos de evento manual para emoji operacional.
- `SYSTEM_ICONS`: mapeamento de 10 labels de vencimento monitorado para emoji operacional.
- `SystemEntry` e `ManualEntry` estendidos com campo `icon: string` — sem mudança na lógica de dados.
- Grade do calendário: indicadores de ícone `text-[8px]` substituem o dot único — até 2 ícones visíveis por dia + `+N` para overflow. Célula `h-[34px]` preservada.
- Lista do dia selecionado: emoji `text-[15px]` + layout duas linhas (label + subtítulo "Vencimento monitorado" ou título curto do evento manual).
- Legenda compacta: frase única `text-[10px]` ao fim do painel de detalhe.
- Bundle `/` 223 kB (inalterado). TypeScript zero erros. Build limpo.
- Sem IA, login, billing, backend, nova aba, nova rota, nova feature fora da AgendaMensal.
